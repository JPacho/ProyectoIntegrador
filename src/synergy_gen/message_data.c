...
#endif
TX_QUEUE thread_display_message_queue;
static uint8_t queue_memory_thread_display_message_queue[SF_MESSAGE_CFG_QUEUE_SIZE];
static sf_message_subscriber_t thread_display_message_queue_0_0 =
{ .p_queue = &thread_display_message_queue, .instance_range =
{ .start = 0, .end = 0 } };
static sf_message_subscriber_t *gp_group_SF_MESSAGE_EVENT_CLASS_TOUCH[] =
{ &thread_display_message_queue_0_0, };
...
void g_message_init(void)
{
    tx_queue_create (&thread_display_message_queue, (CHAR *) "Display Thread Message Queue", 1,
                     &queue_memory_thread_display_message_queue, sizeof(queue_memory_thread_display_message_queue));
}
