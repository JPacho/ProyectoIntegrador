...
void thread_display_create(void);
static void thread_display_func(ULONG thread_input);
static uint8_t thread_display_stack[2048] BSP_PLACE_IN_SECTION_V2(".stack.thread_display") BSP_ALIGN_VARIABLE_V2(BSP_STACK_ALIGNMENT);
...
TX_QUEUE g_main_queue_display;
static uint8_t queue_memory_g_main_queue_display[16];
...
void thread_display_create(void)
{
    ...
    UINT err_g_main_queue_display;
    err_g_main_queue_display = tx_queue_create (&g_main_queue_display, (CHAR *) "Main Queue Display", 4,
                                                &queue_memory_g_main_queue_display,
                                                sizeof(queue_memory_g_main_queue_display));
    if (TX_SUCCESS != err_g_main_queue_display)
    {
        tx_startup_err_callback (&g_main_queue_display, 0);
    }

    UINT err;
    err = tx_thread_create (&thread_display, (CHAR *) "Display Thread", thread_display_func, (ULONG) NULL,
                            &thread_display_stack, 2048, 1, 1, 10, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
        tx_startup_err_callback (&thread_display, 0);
    }
}
