...
void thread_display_entry(void);
...
void thread_display_entry(void) {
   ...
   gx_widget_attach(p_window_root, p_first_screen);
    ...
    while(1)
    {
        ...
        err = g_sf_message0.p_api->pend(g_sf_message0.p_ctrl, &thread_display_message_queue, (sf_message_header_t **) &p_message, TX_WAIT_FOREVER);
        if (err)
        {
            /** TODO: Handle error. */
        }
